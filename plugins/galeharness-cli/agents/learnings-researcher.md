...
<!-- HKT-PATCH:step-0 -->
## Step 0: HKTMemory Vector Search (Run First)
...
- If results are returned: tag each with `(hkt-memory, similarity: X.XX)` and include in your findings
- Merge HKTMemory results with grep results from Steps 1-4 below, deduplicating by title
- HKTMemory results **supplement** but do not replace the `docs/solutions/` grep search
- If the command fails or returns no results, proceed to Step 1 silently
...
- `/gh:plan` - To inform planning with institutional knowledge and add depth during confidence checking
