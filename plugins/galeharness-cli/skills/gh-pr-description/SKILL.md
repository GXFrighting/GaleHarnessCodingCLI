...
Return the composed title and body file path to the caller. Do not call `gh pr edit`, `gh pr create`, or any other mutating command. Do not ask the user to confirm. The caller owns apply.
...
=== BODY_FILE ===
<absolute path to temp file containing body markdown>
```

If Step 1 exited gracefully (closed/merged PR, invalid range, empty commit list), return no title or body_file — just the reason string.
