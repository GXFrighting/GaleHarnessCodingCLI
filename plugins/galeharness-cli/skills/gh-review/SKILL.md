...
The following paths are galeharness-cli pipeline artifacts and must never be flagged for deletion, removal, or gitignore by any reviewer:

- `docs/brainstorms/*` -- requirements documents created by gh:brainstorm
- `docs/plans/*.md` -- plan files created by gh:plan (decision artifacts; execution progress is derived from git, not stored in plan bodies)
...
Use the repository portion of the returned PR URL as `<base-repo>` (for example, `wangrenzhu-ola/GaleHarnessCLI` from `https://github.com/wangrenzhu-ola/GaleHarnessCLI/pull/348`).
...
Use the intent summary as the diff-hygiene anchor: every substantive changed file and behavior change should trace to the stated intent, discovered plan, or an explicit verification need. Unrelated refactors, speculative abstractions, adjacent formatting/comment churn, and unrequested behavior changes are reviewable issues even when the changed code is locally correct.
...
2. Filter to those whose directory is an ancestor of at least one changed file. A standards file governs all files below it (e.g., `plugins/galeharness-cli/AGENTS.md` applies to everything under `plugins/galeharness-cli/`).
...
mkdir -p ".context/galeharness-cli/gh-review/$RUN_ID"
```

Pass `{run_id}` to every persona sub-agent so they can write their full analysis to `.context/galeharness-cli/gh-review/{run_id}/{reviewer_name}.json`.
...
Each persona sub-agent writes full JSON (all schema fields) to `.context/galeharness-cli/gh-review/{run_id}/{reviewer_name}.json` and returns compact JSON with merge-tier fields only:
...
3b. **Diff Hygiene.** Include when any changed file, refactor, abstraction, formatting/comment churn, or behavior change lacks a clear tie to the intent summary, plan requirement, implementation unit, or verification need. Distinguish necessary cleanup from drive-by cleanup: cleanup caused by this diff is acceptable; pre-existing dead code or unrelated quality issues should be reported, not silently fixed.
...
Artifact: .context/galeharness-cli/gh-review/<run-id>/
...
**Detail enrichment (headless only):** The headless envelope includes `Why:`, `Evidence:`, and `Suggested fix:` lines. After merge (Stage 5), read the per-agent artifact files from `.context/galeharness-cli/gh-review/{run_id}/` for only the findings that survived dedup and confidence gating.
...
- Ask a policy question **using the platform's blocking question tool** (`AskUserQuestion` in Claude Code, `request_user_input` in Codex, `ask_user` in Gemini, `ask_user` in Pi (requires the `pi-ask-user` extension)) only when `gated_auto` or `manual` findings remain after safe fixes. Do not replace with a conversational open-ended question. Adapt the options to match what actually remains:
...
- In interactive, autofix, and headless modes, write a per-run artifact under `.context/galeharness-cli/gh-review/<run-id>/` containing:
